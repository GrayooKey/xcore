...
(function(factory){
	...
	$.cxColor = function(){
		...
		colorPanel.build = function(){
			...
			var selectColors = ["FFB900","FF8C00","F7630C","CA5010","DA3B01","EF6950","D13438","D13438","E74856","E81123","EA005E","EA005E","E3008C","BF0077","C239B3","9A0089","0078D7","0063B1","8E8CD8","6B69D6","8764B8","744DA9","B146C2","881798","0099BC","2D7D9A","00B7C3","038387","00B294","018574","00CC6A","00CC6A","7A7574","5D5A58","68768A","515C6B","567C73","486860","498205","107C10"];
			var _html = '';
			...
			for(var i=0; i<selectColors.length; i++){
				...
				_html+="<td title='#"+selectColors[i]+"' style='background-color:#"+selectColors[i]+"'>";
				if((i+1)%8 === 0){
					_html+="</tr>";
				}
			}
			...
			_this.dom.colorTable.on('click', 'td', function(){
				...
				//回填已选颜色
				$("#selectedColorRGB").val(this.title);
			});
