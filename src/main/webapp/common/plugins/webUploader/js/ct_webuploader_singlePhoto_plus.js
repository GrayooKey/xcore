...
let uploaderSinglePhoto;	//Web Uploader实例


let singlePhotoDiv = $(".ct_uploadSinglePhoto_plus"),    //整个上传区域
	photoList = $('<ul class="photoList"></ul>').appendTo(singlePhotoDiv.find(".queueList")),   //将图片列表ul 放到 虚线方框区域中
	placeholder = singlePhotoDiv.find(".placeholder"),     //选择图片输入
	stateSign = "";      //状态标记
...
jQuery(function() {
	...
	// 初始化Web Uploader
	uploaderSinglePhoto = WebUploader.create({
		// swf文件路径
		...
		pick: {
			id: ".filePickerSingle",		//指定选择文件的按钮容器，不指定则不创建按钮。
		...
		fileSingleSizeLimit: typeof(limitSinglePhotoSize)!="undefined" ? limitSinglePhotoSize : 2*1024*1024,
		// 指定接受哪些类型的文件, 可选
		accept: {
			title: 'Images',                    //文字描述
	...
	uploaderSinglePhoto.on('beforeFileQueued', function(file) {
		let $photoList = getPhotoListObj(file);
		let index = $photoList.find("li").size();	//图片上传列表中已有图片数量(index)

		//限制文件上传数量(包括已上传和待上传), 若编辑页面无指定(limitFileNum), 则默认是1
		if(typeof(limitFileNum)!="undefined" ? index>=limitFileNum : index>=1){
			parent.layer.msg('注:此处上传的图片数量不能超过'+ (typeof(limitFileNum)!="undefined" ? limitFileNum : 1) +'张！', {icon: 0});
			return false;
		}

		// 限制单个文件的大小, 超出则不允许加入队列。此处默认是 2*1024*1024 单位:Bytes  (即上传限制默认为2MB)
		if(typeof(limitSinglePhotoSize)!="undefined" ? file.size>limitSinglePhotoSize : file.size>2*1024*1024){
			parent.layer.msg('注:此处上传的单张图片大小不能超过'+  (typeof(limitSinglePhotoSize)!="undefined" ? WebUploader.formatSize(limitSinglePhotoSize) : WebUploader.formatSize(2*1024*1024))  +'！', {icon: 0});
			return false;
		}
	});
	...
	// 当文件被加入队列以后触发
	uploaderSinglePhoto.on('fileQueued', function(file){
		let $photoList = getPhotoListObj(file);

		areaState($photoList, "ready");	//各区域状态改变
		addFile(file);  			//增加图片
	});
});
...
function areaState($photoList, state){
	stateSign = state;		//状态标记值改变
	switch (state){
		case "ready":			//上传队列或图片列表中有图片的状态
			$photoList.siblings(".placeholder").addClass("element-invisible"),				//隐藏 选择图片输入区
			$photoList.show(),										//显示 图片列表ul
			uploaderSinglePhoto.refresh();										//刷新容器(该方法是webuploader自带的刷新方法)
			break;
		case "empty":			//当所有文件移除后的状态
			$photoList.siblings(".placeholder").removeClass("element-invisible"),			//显示 选择图片输入区
...
function addFile(selectFile,file,src,id,$photoList){
	...
	if(src==null||src==""){
		...
		// 文件状态变化时触发
		selectFile.on("statuschange", function(imgP, uploaderSinglePhoto) {
		...
		pd.on("click", ".cancel", function() {
			on_delTempAttach_single(li, selectFile);	//删除待上传附件
		});

		$photoList = getPhotoListObj(selectFile);
		...
		// 图片右上角的删除标志点击事件
		pd.on("click", ".cancel", function() {
			on_delAttach_single(li, file);	//删除已上传附件
		});
...
//获得附件 (根据附件ids获取已上传附件)
function querySinglePhoto(attachIds){
...
function on_delAttach_single(obj, file){
	let url;
	//根据 attachType 判断附件保存在 "um_attach"表 或 "sub_attach"表
	if (typeof (attachType) != "undefined" && attachType == "urmsAttach") {
		...
		$.ajax({
			...
			success : function(data){
				if(data.result){
					...
					let $photoList = $(photoList);  //DOM对象转换成jQuery对象

					obj.remove();			//删除页面元素li

					//各区域状态改变
					if ($photoList.find("li").length > 0) {
						areaState($photoList, "ready");
					} else {
						areaState($photoList,"empty");
					}
...
function on_delTempAttach_single(obj, selectFile){
	parent.layer.confirm("确定删除图片吗？", {
		btn: ["确定","取消"] //按钮
	}, function(){
		let photoList = obj.parent();
		...
		uploaderSinglePhoto.removeFile(selectFile,true);	//删除上传队列中的文件
