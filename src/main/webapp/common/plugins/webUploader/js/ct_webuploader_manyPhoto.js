...
jQuery(function() {
	...
	uploader = WebUploader.create({
	    ...
	    fileNumLimit: typeof(limitFileNum)!="undefined" ? limitFileNum : 4,
  	...
  	if(attachList.length>0){
		areaState("ready");     //各区域状态改变
  		for (var i = 0; i < attachList.length; i++) {
  			photoNum++;                          //图片数量+1
  			photoSizes += attachList[i].attachSize;    //文件总大小叠加
	...
	uploader.on('beforeFileQueued', function(file) {
		//限制文件上传数量(包括已上传和待上传), 若编辑页面无指定(limitFileNum), 则默认是4
		if(typeof(limitFileNum)!="undefined" ? photoNum>=limitFileNum : photoNum>=4){
			parent.layer.msg('注:此处上传的图片数量不能超过'+ (typeof(limitFileNum)!="undefined" ? limitFileNum : 4) +'张！', {icon: 0});
			return false;
		}

		// 限制单个文件的大小, 超出则不允许加入队列。此处默认是 2*1024*1024 单位:Bytes  (即上传限制默认为2MB)
		if(typeof(limitFileSingleSize)!="undefined" ? file.size>limitFileSingleSize : file.size>2*1024*1024){
	...
	switch (state){
		case "ready":			//上传队列或图片列表中有图片的状态
			placeholder.addClass("element-invisible"),				//隐藏 选择图片输入区
				...
				$("#filePickerAdd").removeClass("element-invisible"),	//显示 '继续添加'按钮
				// $(".queueList").addClass("filled"),						//修改虚线方框容器样式
				statusBar.removeClass("element-invisible"),				//显示 状态显示条
		...
		case "empty":			//当所有文件移除后的状态
			placeholder.removeClass("element-invisible"),			//显示 选择图片输入区
				...
				$("#filePickerAdd").addClass("element-invisible"),		//隐藏'继续添加'按钮
				// $(".queueList").removeClass("filled"),					//修改虚线方框容器样式
				statusBar.addClass("element-invisible"),				//隐藏 状态显示条
				uploader.refresh();										//刷新容器(该方法是webuploader自带的刷新方法)
			break;
...
function on_delTempAttach(obj, selectFile){
	parent.layer.confirm("确定删除图片吗？", {
		...
		obj.remove();			//删除页面元素li
		photoNum--;				//图片数量-1
		photoSizes -= selectFile.size;//文件总大小缩减
